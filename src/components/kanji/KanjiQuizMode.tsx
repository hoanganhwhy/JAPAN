...
export default function KanjiQuizMode({ kanjiData, onBack }: Props) {
  const [currentIndex, setCurrentIndex] = useState(0)
  const [score, setScore] = useState(0)
  const [answered, setAnswered] = useState(false)
  const [selectedAnswer, setSelectedAnswer] = useState<string | null>(null)
  const [questions, setQuestions] = useState<Array<{ question: string, correctAnswer: string, options: string[], type: 'meaning' | 'reading' }>>([])
    ...
    const newQuestions = shuffled.map(kanji => {
      ...
      if (questionType === 'meaning') {
        return {
          ...
          options: generateMeaningOptions(kanji, kanjiData),
          type: 'meaning' as const
        }
      } else {
        return {
          question: `${kanji.kanji} (${kanji.meaning})`,
          correctAnswer: kanji.onyomi.split('、')[0],
          options: generateReadingOptions(kanji, kanjiData),
  ...
  const generateMeaningOptions = (correctKanji: KanjiItem, allKanji: KanjiItem[]): string[] => {
  ...
  const generateReadingOptions = (correctKanji: KanjiItem, allKanji: KanjiItem[]): string[] => {
    const correctReading = correctKanji.onyomi.split('、')[0]
    const options = [correctReading]
    const otherKanji = allKanji.filter(k => k.onyomi !== correctKanji.onyomi)
    
    while (options.length < 4 && otherKanji.length > 0) {
      const randomIndex = Math.floor(Math.random() * otherKanji.length)
      const option = otherKanji[randomIndex].onyomi.split('、')[0]
      if (!options.includes(option)) {
        options.push(option)
      }
      otherKanji.splice(randomIndex, 1)
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-2xl font-bold text-[#765534]">Trắc Nghiệm Kanji</h2>
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600">
              {currentQuestion.type === 'meaning' ? 'Chọn nghĩa đúng:' : 'Chọn cách đọc Onyomi (音読み):'}
            </p>
          </div>
