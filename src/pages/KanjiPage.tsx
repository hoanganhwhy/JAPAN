...
import type { FavoriteKanji } from '@/hooks/useFavorites'
...
const KANJI_DATA = [
  ...
  { kanji: '円', onyomi: 'エン',          kunyomi: '',                      meaning: 'yên, tròn',             example: '百円 (ひゃくえん) - một trăm yên' },
...
// Convert raw kanji entry to FavoriteKanji
const toFav = (item: typeof KANJI_DATA[0]): FavoriteKanji => ({
...
export default function KanjiPage() {
  ...
  const { isKanjiFav, toggleKanji } = useFavorites()

  if (selectedMode === 'guess')     return <KanjiGuessMode     kanjiData={KANJI_DATA} onBack={() => setSelectedMode('')} />
  if (selectedMode === 'quiz')      return <KanjiQuizMode      kanjiData={KANJI_DATA} onBack={() => setSelectedMode('')} />
  if (selectedMode === 'flashcard') return <KanjiFlashcardMode kanjiData={KANJI_DATA} onBack={() => setSelectedMode('')} />
  ...
  // ── Learn mode: grid of all 80 kanji ────────────────────────────────────────
      ...
      <div className="min-h-screen py-8 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h2 className="text-2xl font-bold text-[#765534] mb-6 text-center">80 Chữ Kanji N5</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {KANJI_DATA.map((item, index) => {
                const faved = isKanjiFav(`kanji_${item.kanji}`)
                return (
                  <div key={index} className="relative group">
                    <button
                      onClick={() => setSelectedKanji(item)}
                      className="w-full p-6 bg-gradient-to-br from-[#FEE173] to-white rounded-xl hover:shadow-lg transition-all hover:scale-105 border-2 border-transparent hover:border-[#FEC900] text-left"
                    >
                      <div className="text-5xl font-bold text-[#765534] mb-2">{item.kanji}</div>
                      <div className="text-sm text-gray-600">{item.meaning}</div>
                    </button>
                    {/* Star badge on card */}
                    <button
                      onClick={(e) => { e.stopPropagation(); toggleKanji(toFav(item)) }}
                      className={`absolute top-2 right-2 transition-all hover:scale-125 ${
                        faved ? 'text-yellow-500 opacity-100' : 'text-gray-300 opacity-0 group-hover:opacity-100'
                      }`}
                      title={faved ? 'Bỏ yêu thích' : 'Thêm vào yêu thích'}
                    >
                      <Star size={20} fill={faved ? 'currentColor' : 'none'} />
                    </button>
                  </div>
                )
              })}
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"
              ...
              <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-2xl w-full" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-6">
                  <div className="text-8xl font-bold text-[#765534]">{selectedKanji.kanji}</div>
                  <button onClick={() => setSelectedKanji(null)} className="text-gray-400 hover:text-[#765534] text-3xl">×</button>
                </div>
                <div className="space-y-4">
                  ...
                  <button
                    onClick={() => toggleKanji(toFav(selectedKanji))}
                    className={`w-full py-3 rounded-full font-semibold transition-colors flex items-center justify-center gap-2 ${
                      isKanjiFav(`kanji_${selectedKanji.kanji}`)
                        ? 'bg-yellow-500 text-white hover:bg-yellow-600'
                        : 'bg-[#FEC900] text-[#765534] hover:bg-[#FEE173]'
                    }`}
                  >
                    <Star size={20} fill={isKanjiFav(`kanji_${selectedKanji.kanji}`) ? 'currentColor' : 'none'} />
                    {isKanjiFav(`kanji_${selectedKanji.kanji}`) ? 'Đã thêm vào yêu thích ✓' : 'Thêm vào yêu thích'}
                  </button>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen py-8 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-[#765534] mb-4">Kanji N5</h1>
          <p className="text-lg text-gray-600">Học 80 chữ Kanji cơ bản cho kỳ thi JLPT N5</p>
        </div>

        {/* Learning Modes */}
        <div className="mb-12">
          <h2 className="text-2xl font-bold text-[#765534] mb-6 text-center">Chọn Phương Pháp Học</h2>
        ...
        {/* Kanji Chart Preview */}
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-[#765534] mb-6 text-center">Bảng Kanji N5 (80 chữ)</h2>
          <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-3">
            {KANJI_DATA.slice(0, 48).map((item, index) => {
              ...
              return (
                <div key={index} className="relative group">
                  <div
                    onClick={() => { setSelectedMode('learn'); setSelectedKanji(item) }}
                    ...
                    title={item.meaning}
                  >
                    <div className="text-2xl md:text-3xl font-bold text-[#765534]">{item.kanji}</div>
                  </div>
                  {/* Mini star badge */}
                  {faved && (
          ...
          <div className="text-center mt-6">
            <p className="text-gray-500 text-sm mb-4">✨ Hiển thị 48/{KANJI_DATA.length} chữ Kanji</p>
            <button onClick={() => setSelectedMode('learn')}
